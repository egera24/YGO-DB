...
Version: 1.5.0 - FIXED: Chunk-based processing with true graceful shutdown
Architecture: Processes cards in chunks to enable fast interrupt response
"""
...
PHASE1_CHUNK_SIZE = 300  # Process cards in chunks for graceful shutdown
...
def extract_price_data(html):
    """Extract all 5 price fields from HTML with N/A priority check"""
...
def phase1_fast_run(cards, start_idx):
    """PHASE 1: Chunk-based processing for graceful shutdown"""
    print("\n" + "="*70)
    print(" PHASE 1: ULTRA-FAST RUN (Chunk-Based)")
    print("="*70)
    print(f"Workers: {PHASE1_MAX_WORKERS} | Speed: {PHASE1_REQUESTS_PER_SECOND} req/sec")
    print(f"Chunk Size: {PHASE1_CHUNK_SIZE} cards | Retries: {PHASE1_MAX_RETRIES}")
    print(f"Display: Every {DISPLAY_INTERVAL} cards")
    print(f"💡 Press Ctrl+C to stop gracefully (max 30-60s shutdown time)")
    print()
    ...
    # Filter already processed cards
    cards_to_process = []
    for idx, card in enumerate(remaining):
        if card['card_id'] not in processed_ids:
            cards_to_process.append((start_idx + idx, card))
    
    if not cards_to_process:
        print("✓ No new cards to process (all already completed)")
        return (successful_cards, rejections, stats)
    
    total_cards = len(cards_to_process)
    print(f"Cards to process: {total_cards:,}")
    print()
    ...
    try:
        ...
        for chunk_start in range(0, len(cards_to_process), PHASE1_CHUNK_SIZE):
            chunk_number += 1
            chunk_end = min(chunk_start + PHASE1_CHUNK_SIZE, len(cards_to_process))
            chunk_cards = cards_to_process[chunk_start:chunk_end]
            
            # Submit all futures for this chunk
            with ThreadPoolExecutor(max_workers=PHASE1_MAX_WORKERS) as executor:
                futures = {}
                
                for idx_in_chunk, (absolute_idx, card) in enumerate(chunk_cards):
                    worker_id = idx_in_chunk % PHASE1_MAX_WORKERS
                    future = executor.submit(
                        process_card_worker, worker_id, card, absolute_idx,
                        session_pool, rate_limiter, PHASE1_MAX_RETRIES, [], False
                    )
                    futures[future] = (absolute_idx, card)
                
                # Process all futures in this chunk
                for future in as_completed(futures):
                    absolute_idx, card = futures[future]
                    ...
                    try:
                        ...
                        total_completed += 1
                        last_saved_idx = absolute_idx
                        
                        # Display progress
                        if total_completed == 1 or total_completed % DISPLAY_INTERVAL == 0:
                            progress = (total_completed / total_cards) * 100
                            elapsed = (datetime.now() - start_time).total_seconds()
                            rate = total_completed / elapsed * 3600 if elapsed > 0 else 0
                            eta_seconds = (total_cards - total_completed) / total_completed * elapsed if total_completed > 0 else 999999
                            eta_hours = int(eta_seconds // 3600)
                            eta_minutes = int((eta_seconds % 3600) // 60)
                            
                            eta_display = "Calculating..." if total_completed < 100 else f"{eta_hours}h {eta_minutes}m"
                            ...
                            print(f"[{total_completed:5}/{total_cards:5}] {status_text:40} | {progress:5.1f}% | "
                        ...
                        if total_completed % SAVE_INTERVAL == 0:
                            checkpoint = {'last_processed_index': absolute_idx, 'phase1_complete': False}
            ...
            # Chunk completed - save checkpoint
            checkpoint = {'last_processed_index': last_saved_idx, 'phase1_complete': False}
            save_data(successful_cards, rejections, checkpoint)
            
            # Check for interrupt BETWEEN chunks (safe point)
            # This is where KeyboardInterrupt will be caught naturally
    
    except KeyboardInterrupt:
        interrupted = True
        print(f"\n\n⚠️  Interrupt received after completing chunk #{chunk_number}")
        print(f"   Finishing current chunk... (may take up to 60 seconds)")
    
    # Save final state
    checkpoint = {
        'last_processed_index': last_saved_idx, 
        'phase1_complete': not interrupted
    }
    save_data(successful_cards, rejections, checkpoint)
    ...
    if interrupted:
        ...
        print(f"{'='*70}")
        print(f"✓ Completed {chunk_number} chunks ({chunk_number * PHASE1_CHUNK_SIZE} cards processed)")
        print(f"✓ Progress saved to card index: {last_saved_idx}")
        print(f"✓ Total processed: {total_completed} cards")
    ...
    print(f"{'='*70}")
    print(f"Processed: {total_completed}")
    print(f"Successful: {stats['success']}")
    ...
    if duration.total_seconds() > 0:
        print(f"Speed: {total_completed / (duration.total_seconds() / 3600):.0f} cards/hour")
    
    return (successful_cards, rejections, stats)


def phase2_recovery(rejections, successful_cards):
    """PHASE 2: Careful recovery with interrupt support"""
    ...
    try:
        ...
        print(f"\n⚠️  Phase 2 interrupted - saving progress...")
        still_rejected.extend(recoverable[idx+1:])
...
def main():
    """Main execution with graceful interrupt handling"""
    print("="*70)
    print(" CARDMARKET CARD DETAILS SCRAPER v1.5.0")
    print("="*70)
    print()
    ...
    if not checkpoint.get('phase1_complete', False):
        result = phase1_fast_run(cards, start_idx)
        
        if result is None:
            print("\n✓ Script interrupted gracefully. All progress saved.")
            return
        
        successful_cards, rejections, stats = result
    else:
    ...
    hours = overall_duration.total_seconds() / 3600
    if hours > 0:
        print(f"Speed: {len(cards) / hours:.0f} cards/hour")
    print("="*70)
    print("\n✓ Complete!")
...
if __name__ == '__main__':
    try:
        ...
        print("\n\n⚠️  Script terminated during startup/loading")
    except Exception as e:
        print(f"\n✗ Unexpected error: {e}")
        import traceback
        traceback.print_exc()
