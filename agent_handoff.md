...
- **Render prod keep-alive** is **not** a GHA workflow — use [cron-job.org](https://cron-job.org/en/) per [`docs/render-keepalive.md`](docs/render-keepalive.md) (every 10 min, Europe/Budapest 08:00–22:00).
...
R2 bucket `ygo-cardmarket`: compressed archives under `archives/{YYYY}/{MM}/{DD}/{HHMM}/` (`catalog_archive.zip`, `cardmarket_prices.zip`, `sync_price_log.log.br`, `sync_price_report.json.br`). Prices: SCD Type 2 in `printing_market_prices` (`is_current = true` for UI reads).
