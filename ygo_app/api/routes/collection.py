...
from ygo_app.services import add_collection_item, list_collection
from ygo_app.config import ROOT_DIR
import tempfile
...
def get_collection(
    ...
    items, _total = list_collection(
        db, q=q, folder=folder, set_code=set_code, limit=limit, offset=offset
    )
    return [CollectionItemOut(**item) for item in items]


@router.post("", response_model=CollectionItemOut)
def create_item(body: CollectionItemCreate, db: Session = Depends(get_db)):
    item = add_collection_item(db, body.model_dump())
    card_id = None
    image = None
    from ygo_app.services import find_card_by_set_code

    card = find_card_by_set_code(db, item.set_code)
    if card:
        card_id = card.id
        image = card.image_url_small
...
def update_item(
    item_id: int, body: CollectionItemUpdate, db: Session = Depends(get_db)
):
...
async def import_csv(file: UploadFile | None = None, replace: bool = True):
    if file and file.filename:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            ...
            tmp.write(content)
            path = tmp.name
        count = import_collection_csv(path, replace=replace)
        return {"imported": count}
    default = ROOT_DIR / "my_collection.csv"
    if not default.exists():
        raise HTTPException(400, "No file uploaded and my_collection.csv not found")
    count = import_collection_csv(default, replace=replace)
    return {"imported": count, "source": str(default)}
